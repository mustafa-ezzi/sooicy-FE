...
const addToCart = (product) => {
  setCart(prev => {
    // Check if the same product with same addons already exists
    const existingIndex = prev.findIndex(item => {
      // Match by product ID
      if (item.id !== product.id) return false;
      
      // Match by addons (if any)
    ...
    return [...prev, { 
      ...
      cartItemId: Date.now(), // Unique identifier for cart item
      selectedAddons: product.selectedAddons || [] // Ensure addons are included
    }];
  });
...
const placeOrder = async (orderDetails) => {
  try {
    ...
    // Prepare items_data with addons
    ...
    try {
      const orderData = {
        ...
        sooicy_user: userResponse?.user?.id || currentUser?.id || null,
        items_data: itemsData // This must not be empty!
      };
